...
def set_calling_name(id, client): #checks whether or not the message.author should be called by a nickname
    global calling_name
    user_data = dm.search_user(id)
    name = ' '
    if((user_data[2] == True) and user_data[1] is not None):
        name += user_data[1]
    else:
        name += compile_name(id)
    if(user_data[5] is not None and user_data[5] != ''):
        name += ' ' + em.id_to_emote(user_data[5],client)
    return name

def set_embed_calling_name(author, client): #checks whether or not the message.author should be called by a nickname
...
def nickname_set(message,author): #sets the author's nickname
    ...
    if nickname.strip() != '':
        if check_badwords(nickname.lower()):
            return('WHAT? I cannot call you that, ' + compile_name(author.id) + '!')
        else:
            dm.update_value(user_data[0],'petname',nickname)
            dm.update_value(user_data[0],'referby',1)
            return ('Alright! I got your nickname now, ' + nickname + '!')
    else:
        return (compile_name(author.id) + ", I can't call you nothing!")
